...
def get_stones_on_board(sgf_string, folder, file):
    def analyse_node(node):
        ...
        new_game = BaseGame(position_node)
        new_game.write_sgf(os.path.join(folder, f'{file.rsplit(".", 1)[0]}-trimmed.sgf'))
...
root_path = r"C:\Users\StanislawFrejlak\Go\lessons\cutting_stones"
for file in os.listdir(root_path):
    filepath = os.path.join(root_path, file)
    with open(filepath, 'r') as f:
        sgf_string = f.read()
    get_stones_on_board(sgf_string, root_path, file)
