...
class BaseEngine:  # some common elements between analysis and contribute engine
    ...
    def on_error(self, message, code, allow_popup=False):
...
class KataGoEngine(BaseEngine):
    ...
    def __init__(self, katrain, config, custom_logfile=None):
        ...
        if config.get("altcommand", ""):
            ...
            if not os.path.isfile(model):
                self.on_error("Kata model not found") #.format(model=model), code="KATAGO-FILES")
                return  # don't start
            if not os.path.isfile(cfg):
                self.on_error("Kata config not found") #.format(config=cfg), code="KATAGO-FILES")
                return  # don't start
            log_file_command = "" if custom_logfile is None else f", logFile={custom_logfile}"
            self.command = shlex.split(
                f'"{exe}" analysis -model "{model}" -config "{cfg}" -override-config "homeDataDir={os.path.expanduser(DATA_FOLDER)}{log_file_command}"'
            )
        # -analysis-threads {config["threads"]}
        self.start()
    ...
    def start(self):
        with self.thread_lock:
            ...
            try:
                ...
                self.on_error("Starting Kata failed") #.format(command=self.command, error=e), code="c")
                return  # don't start
    ...
    def check_alive(self, os_error="", exception_if_dead=False, maybe_open_recovery=False):
        ...
        if not ok and exception_if_dead:
            if self.katago_process:
                ...
                if code == 3221225781:
                    died_msg = "Engine missing DLL"
                else:
                    died_msg = "Engine died unexpectedly" #).format(error=f"{os_error} status {code}")
                ...
                self.katrain.log("Engine died unexpectedly") #.format(error=os_error), OUTPUT_DEBUG)
        return ok
    ...
    def _write_stdin_thread(self):  # flush only in a thread since it returns only when the other program reads
        while self.katago_process is not None:
            try:
                queue_item = self.write_queue.get(block=True, timeout=0.1)
                query, callback, error_callback, next_move, node = queue_item
            except queue.Empty:
                continue
            with self.thread_lock:
                if "id" not in query or query["id"] is None:  # SF: Added the None condition
